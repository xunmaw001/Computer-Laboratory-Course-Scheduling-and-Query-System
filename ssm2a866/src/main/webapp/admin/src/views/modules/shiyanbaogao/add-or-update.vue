<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info' && !ro.xueshengbaogao" label="学生报告" prop="xueshengbaogao">
          <file-upload
          tip="点击上传学生报告"
          action="file/upload"
          :limit="3"
          :multiple="true"
          :fileUrls="ruleForm.xueshengbaogao?ruleForm.xueshengbaogao:''"
          @change="xueshengbaogaoUploadChange"
          ></file-upload>
        </el-form-item>
        <div v-else>
          <el-form-item v-if="ruleForm.xueshengbaogao" label="学生报告" prop="xueshengbaogao">
            <img style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.xueshengbaogao.split(',')" :src="item" width="100" height="100">
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
  ...
  methods: {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `shiyanbaogao/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.shiyanbaogaoCrossAddOrUpdateFlag = false;
                  this.parent.search();
    ...
    xueshengbaogaoUploadChange(fileUrls) {
	this.ruleForm.xueshengbaogao = fileUrls;
			this.addEditUploadStyleChange()
    },
	addEditStyleChange() {
	  this.$nextTick(()=>{
